...
info_retry:
	...
	NVT_ERR("%s:ts->fw_ver=%d\n",  __func__, ts->fw_ver);
	NVT_ERR("%s:ts->x_num=%d\n", __func__, ts->x_num);
	NVT_ERR("%s:ts->y_num=%d\n", __func__, ts->y_num);
	NVT_ERR("%s:ts->abs_x_max=%d\n", __func__, ts->abs_x_max);
	NVT_ERR("%s:ts->abs_y_max=%d\n", __func__, ts->abs_y_max);
	NVT_ERR("%s:ts->max_button_num=%d\n", __func__, ts->max_button_num);
...
void nvt_ts_wakeup_gesture_report(uint8_t gesture_id, uint8_t *data)
{
	...
	switch (gesture_id) {
		...
		case GESTURE_DOUBLE_CLICK:
			...
			ts->dbclick_count++;
			snprintf(ch, sizeof(ch), "%d", ts->dbclick_count);
...
static const char *nvt_get_config(struct nvt_ts_data *ts)
{
	...
	for (i = 0; i < ts->config_array_size; i++) {
		NVT_LOG("lockdown_info[0] = %d config_array[%d].tp_vendor = %d\n", ts->lockdown_info[0], i, ts->config_array[i].tp_vendor);
		NVT_LOG("lockdown_info[3] = %d config_array[%d].tp_hw_version = %d\n", ts->lockdown_info[3], i, ts->config_array[i].tp_hw_version);
	...
	if (ts->dump_click_count) {
		...
		//add_hw_monitor_info(HWMON_CONPONENT_NAME, "tpvendor", ts->current_clicknum_file);
	}
#endif
...
static int32_t nvt_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	//add_hw_component_info(HWCOMPONENT_NAME_TOUCH, HWCOMPONENT_KEY_IC, "Novatek");
	...
	if (tp_maker == NULL)
		...
		//strlcpy(tp_maker, update_hw_component_touch_module_info(ts->lockdown_info[0]), 20);
